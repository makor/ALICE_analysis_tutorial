...
void AliAnalysisTaskMyTask::UserExec(Option_t *) {
  ...
  for (int iGamma = 0; iGamma < fReaderGammas->GetEntriesFast(); ++iGamma) {
    ...
    if (fMCEvent) {
      ...
      TParticle *posDaughter = PhotonCandidate->GetPositiveMCDaughter(fMCEvent);
      TParticle *negDaughter = PhotonCandidate->GetNegativeMCDaughter(fMCEvent);
      ...
      if (posDaughter == NULL || negDaughter == NULL) {
        fKind ->Fill(1) ;  // garbage
                    // 				cout << "one of the daughters not available" <<
                    // endl;
      } else if (posDaughter->GetMother(0) != negDaughter->GetMother(0) ||
                 (posDaughter->GetMother(0) == negDaughter->GetMother(0) &&
                  posDaughter->GetMother(0) == -1)) {
        ...
        if (negDaughter->GetPdgCode())
          ...
          continue;
        // 				cout << "PDG codes daughters: " << pdgCodePos <<
        // "\t" << pdgCodeNeg << endl;
        ...
        // "\t" << pdgCodeNeg << endl;
        Int_t pdgCode;
        if (PhotonCandidate->GetMCParticle(fMCEvent)->GetPdgCode())
          pdgCode = PhotonCandidate->GetMCParticle(fMCEvent)->GetPdgCode();
        else
          continue;
        ...
        if (TMath::Abs(pdgCodePos) != 11 || TMath::Abs(pdgCodeNeg) != 11)
          ...
          Bool_t gammaIsPrimary = fEventCuts->IsConversionPrimaryAOD(
              fMCEvent, posDaughter->GetMother(), mcProdVtxX, mcProdVtxY,
              mcProdVtxZ);
          if (pdgCode == 111)
    ...
    *>(PhotonCandidate->GetPositiveMCDaughter(fMCEvent));
    ...
    for (auto v0obj : *(fAOD->GetV0s())) {
      ...
      if (fIsMC) {
        ...
        if (!mcarray) continue;
        int daugh[2] = {11,
                        11};  // we check whether the v0 is a photon, i.e. the
    ...
    PostData(1,
             fOutputList);  // stream the results the analysis of this event to
  ...
  void AliAnalysisTaskMyTask::StoreGlobalTrackReference() {
    ...
    for (int iTrack = 0; iTrack < aodEvent->GetNumberOfTracks(); ++iTrack) {
      AliAODTrack *track =
          static_cast<AliAODTrack *>(aodEvent->GetTrack(iTrack));
      if (!track) continue;
